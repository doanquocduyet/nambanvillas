...
- **Trang theo cây trồng** `/ban-vuon-ca-phe-nam-ban/`, `/ban-vuon-bo-nam-ban/`: lô vào trang theo nhãn `ca-phe` / `bo` trong `data-nhan` thẻ hub — CHỈ gắn khi vườn/cây nằm TRÊN chính lô (xem `docs/package/MO-O-DANG-TIN.md`).
- `scripts/cap-nhat-gia-hom-nay.py` — **GIÁ ĐẤT HÔM NAY tự tính từ lô thật** (P10–P90 + trung vị, lịch sử `data/gia-tuan.json`). Chạy bằng GitHub Actions `gia-tuan.yml`: NGAY khi hub đổi + mỗi thứ Hai. Mỗi tuần 1 ô (trong tuần làm mới, sang tuần thêm ô), KHÔNG xoá ô cũ. Không bao giờ điền số tay vào bảng tuần.
