...
- `docs/DANG-CUM-MOI.md` — **HƯỚNG DẪN ĐĂNG CỤM MỚI**: khi có 1 CỤM (nhiều nền cùng khu) → tạo trang `/dat-nen/cum-<slug>/` + nén ảnh + chèn card vào `cum-moi-nam-ban/index.html` (mục "Cụm đang mở") + thêm sitemap. Tối đa 4 card, cụm mới trên cùng, bán hết thì gỡ. Lô lẻ KHÔNG đưa vào trang Cụm Mới. **Cuối file: chuẩn KEYWORD + AEO/SEO/GEO cho cụm — mỗi cụm mới cập nhật ItemList + dải giá của hub, link ngược hub.**
...
- `scripts/cap-nhat-gia-hom-nay.py` — **GIÁ ĐẤT HÔM NAY tự tính từ lô thật** (P10–P90 + trung vị, lịch sử `data/gia-tuan.json`), Routine chạy mỗi thứ Hai. Không bao giờ điền số tay vào bảng tuần.
