...
- **[GIÁ ĐẤT HÔM NAY — TIN TRƯỚC, TỔNG HỢP SAU — CHỐT 25/9/2026]** Mỗi tuần trên `/thi-truong/gia-dat-nam-ban-hom-nay/`: **tuần đang chạy**: tin rao trong tuần → lô mới lên Nam Ban Villas → **Tổng hợp tuần là mục CUỐI**. **Tuần đã qua**: Tổng hợp tuần lên NGAY DƯỚI tiêu đề tuần (xem lại đọc nhanh), tin rao + lô mới ở dưới cho ai cần chi tiết. Script `cap-nhat-gia-hom-nay.py` (hàm `tong_hop_len_dau`) tự đảo khi sang tuần mới. Mỗi tuần 1 ô: trong tuần chạy lại thì làm mới ô đó, không sinh ô trùng.
- **[VILLAS LÀ Ô DUY NHẤT QUÉT TIN RAO — CHỐT 26/9/2026]** Villas quét sâu và nhiều nhất mọi nguồn (guland, datnenlamdong, batdongsanonline, thuviennhadat, muaban, mogi, Chợ Tốt, YouTube, Facebook công khai…), lọc kỹ rồi đăng lên `/thi-truong/tin-rao-dat-nam-ban-moi/`. Panorama KHÔNG tự quét nữa — chỉ đọc trang tin rao của Villas mỗi Chủ nhật để tổng hợp vào Namban Index. Không làm trùng nhiệm vụ. Vì Panorama đọc máy trang này: **giữ nguyên khuôn** (`<!-- DAY:YYYY-MM-DD -->`, `li.tin-item`, `.tin-specs` span đầu = diện tích, có 1 span tổng giá, span cuối = vị trí; tin số lệch ghi "lệch nhau" trong `.tin-desc`). Muốn đổi khuôn phải báo ô Panorama trước. Tin Nam Hà giữ, ghi "Nam Hà (giáp Nam Ban), Lâm Hà". Cách quét (5 cửa/sàn, dò phân trang, đọc theo tầng, gộp trùng, luật dừng): `docs/package/KINH-NGHIEM-QUET-TIN-TU-PANORAMA.md`.
- **[HAI WEB ĐỘC LẬP CÙNG KEY LÀ BÌNH THƯỜNG — CHỐT 26/9/2026, chủ web: "từ xưa giờ luôn vậy"]** Villas và Panorama là 2 tên miền độc lập: CÙNG nhắm một key và cùng lên trang 1 là TỐT (chiếm 2 chỗ). KHÔNG ép bên nào đổi tựa để "nhường key". "Tự cắn key" chỉ xét GIỮA CÁC TRANG TRONG CÙNG 1 WEB. Giữa 2 web chỉ cần: (1) nội dung khác góc, không chép dàn ý/câu chữ — Villas = lô thật, giá thật, dịch vụ, nút gọi; Panorama = phân tích, số Index, hướng dẫn; (2) Villas KHÔNG link sang Panorama. Số giá chủ web đưa: đo đạc 250–400 nghìn/mốc; máy múc nhỏ tới lớn 400–800 nghìn/giờ (bao công + dầu) — xe lu, xe ben chưa có số riêng; cột điện bê tông 2,4–3 triệu/cây; đèn NLMT 1–3 triệu/cái.
...
- **[ĐÓNG DẤU BẢN QUYỀN MỌI ẢNH — CHỐT 25/9/2026, chú dặn]** TẤT CẢ ảnh chú đưa lên web phải mang dấu © Nam Ban Villas TRONG FILE (EXIF + XMP: bản quyền, tác giả, link web, hotline — KHÔNG ghi GPS). 5 script ảnh (`prep-anh` · `nen-anh` · `tao-webp` · `tao-srcset` · `xoa-dau-do`) tự đóng. Ảnh xử lý bằng cách khác (cắt tay, xoay, che PII bằng PIL, chép thẳng vào `images/`) → chạy `python3 scripts/dong-dau-anh.py <thư-mục>`. Checker mục 22 chặn ảnh thiếu dấu.
- **[DẤU © TRONG FILE ẢNH — CHỐT 25/9/2026]** Mọi ảnh trong `images/` mang EXIF+XMP "© Nam Ban Villas — nambanvillas.vn" (như Panorama). `nen-anh.py`/`prep-anh.py` tự đóng sau khi nén; ảnh chép thẳng vào thì chạy `python3 scripts/dong-dau-anh.py`. Checker mục 22 chặn ảnh thiếu dấu. Không bao giờ ghi GPS vào ảnh.
